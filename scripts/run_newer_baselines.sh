#!/usr/bin/env bash
# Round-38 sweep: 5 newer kvpress baselines × 4 NIAH adversarial
# subtasks at 32K, n=20 each, on Qwen 2.5-7B-Instruct.
#
# Runs 2 cells in parallel (one per A100). Total wall ~25-40 min for
# the 20-cell matrix on 2x A100 80GB.
#
# Output: experiments/newer_baselines/<method>_<subtask>_32k/
#         {summary.json, preds.jsonl}
...
export CUDA_MPS_PIPE_DIRECTORY=/tmp/no-mps-r38

OUTROOT=experiments/newer_baselines
mkdir -p "${OUTROOT}/_logs"

METHODS=(expected_attention tova pyramidkv think adakv)
SUBTASKS=(niah_multikey_1 niah_multikey_2 niah_multivalue niah_multiquery)

# Queue all (method, subtask) tuples
QUEUE=()
for m in "${METHODS[@]}"; do
...
# Run 2 cells in parallel, one per GPU
PIDS=()
GPUS=(0 1)
GPU_IDX=0
